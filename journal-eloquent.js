...
for (let i = 0; i < JOURNAL.length; i++) {
  ...
  console.log(`${entry.events.length} events for loop`);
}
// same effect, more concise:
for (const entry of JOURNAL) {
  console.log(`${entry.events.length} events`);
}

// create two-by-two table for a journal event
...
// compute phi coefficient from array
function phi(table) {
  return (
...
// create an array of all events in journal
...
// using array of all events calculate phi coefficient for each
for (let event of journalEvents(JOURNAL)) {
  console.log(event + ":", phi(tableFor(event, JOURNAL)));
}

// get just correlations greater than 0.1 or less than -0.1 to look for statistically significant events
for (let event of journalEvents(JOURNAL)) {
